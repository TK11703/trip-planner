...
{
    Task<TripListResponse> GetPageAsync(string ownerUserId, int page, int pageSize, CancellationToken cancellationToken);
...
public sealed class TripReadRepository : ITripReadRepository
{
    ...
    public async Task<TripListResponse> GetPageAsync(string ownerUserId, int page, int pageSize, CancellationToken cancellationToken)
    {
        ...
        var rows = (await conn.QueryAsync<TripSummaryPageRow>(new CommandDefinition(query, new { OwnerUserId = ownerUserId, Limit = pageSize, Offset = offset }, cancellationToken: cancellationToken))).ToArray();
        var total = rows.FirstOrDefault()?.TotalCount ?? 0;
        var trips = rows.Select(r => new TripSummary(r.TripId, r.Name, r.StartDate, r.EndDate, r.UpdatedAtUtc, r.ItemCount)).ToArray();
    ...
    private sealed record TripSummaryPageRow(Guid TripId, string Name, DateOnly StartDate, DateOnly EndDate, DateTimeOffset UpdatedAtUtc, int ItemCount, int TotalCount);
