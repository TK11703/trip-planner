-- Feature 021: Email Item Ingestion.
-- Raw inbox emails received from ACS/Event Grid, a deduplication hash,
-- and parsed item drafts awaiting user review before promotion to TrackedItems.
...
CREATE TABLE IF NOT EXISTS parsed_item_drafts (
    parsed_item_draft_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    ...
    created_at_utc timestamptz NOT NULL DEFAULT timezone('utc', now()),
    CONSTRAINT parsed_item_drafts_review_status_chk CHECK (review_status IN ('pending_review','confirmed','discarded'))
);

-- Pending drafts by user for the review queue.
CREATE INDEX IF NOT EXISTS parsed_item_drafts_user_pending_idx
    ON parsed_item_drafts (user_id, created_at_utc DESC)
