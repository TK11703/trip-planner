...
-- @Insert
INSERT INTO tracked_items (tracked_item_id, trip_id, trip_leg_id, owner_user_id, item_type, title, location, start_local, start_time_zone_id, starts_at, end_local, end_time_zone_id, ends_at, display_color, confirmation_code, notes, estimated_cost, sort_order, created_at_utc, updated_at_utc)
SELECT @TrackedItemId, @TripId, @TripLegId, @OwnerUserId, @ItemType, @Title, @Location, @StartLocal, @StartTimeZoneId, @StartsAt, @EndLocal, @EndTimeZoneId, @EndsAt, @DisplayColor, @ConfirmationCode, @Notes, @EstimatedCost, @SortOrder, @NowUtc, @NowUtc
...
SET trip_leg_id = @TripLegId, item_type = @ItemType, title = @Title, location = @Location,
    ...
    confirmation_code = @ConfirmationCode, notes = @Notes, estimated_cost = @EstimatedCost, sort_order = @SortOrder
