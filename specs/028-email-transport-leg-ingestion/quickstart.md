...
- [X] Every leg created from email passes `TripLegForm` validation unchanged (SC-003) *(walked: both email-created legs — a Flight and a Train — were reopened in `TripLegForm` and saved with no change. Every field bound, no validation message appeared, and each row's `updated_at_utc` advanced while every other column stayed byte-identical, so the save was a real write rather than a silent refusal)*
